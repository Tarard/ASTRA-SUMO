...
def test_plugin_install_doc_explains_marketplace_and_new_thread() -> None:
    ...
    assert "codex plugin marketplace add Tarard/ASTRA-SUMO --ref main" in doc
    assert "codex plugin add astra-sumo@astra-sumo" in doc
    assert "Start a new Codex thread" in doc
    assert "plugins/astra-sumo" in doc
    assert "plugins/astra-sumo/.codex-plugin/plugin.json" in doc
    assert "skill is the reasoning layer" in doc
    assert "MCP server is the execution layer" in doc
    assert "astra_auto_workflow" in doc
