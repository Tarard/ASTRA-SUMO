...
for name in ("geometry", "source", "boundaries", "groups", "lanes", "movements"):
    importlib.import_module("astra_sumo.core.hamburg_junctions." + name)
assert "astra_sumo.core.hamburg_aerial_corridor_candidate" not in sys.modules
from astra_sumo.core.hamburg_junctions.geometry import fit_movement_shape_to_anchors
shape, error = fit_movement_shape_to_anchors([(0, 0), (5, 0), (10, 0)], start=(2, 0), end=(8, 0))
assert shape == [(2, 0), (5.0, 0.0), (8, 0)] and error == 0
from astra_sumo.core.hamburg_aerial_corridor_candidate import fit_movement_shape_to_anchors as original
...
for name in ("network", "geometry", "signatures", "parity", "scope", "edge_mapping",
    ...
    importlib.import_module("astra_sumo.core.junction_rebuild." + name)
for name in ("junction_rebuild_candidate", "junction_rebuild_tail", "junction_rebuild_helpers"):
    assert "astra_sumo.core." + name not in sys.modules
from astra_sumo.core.junction_rebuild.network import _connection_key
assert _connection_key(ET.Element("connection", {"from": "a", "to": "b", "fromLane": "1", "toLane": "2"})) == ("a", "b", "1", "2")
from astra_sumo.core.junction_rebuild.target_replay import write_teacher_target_internal_replay_net
from astra_sumo.core.junction_rebuild_candidate import write_teacher_target_internal_replay_net as original
