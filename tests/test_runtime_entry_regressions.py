...
from astra_sumo import cli, mcp_contract_tools as contracts
...
def test_blocked_netedit_operation_has_a_failure_summary(monkeypatch, operation):
    ...
    result = getattr(contracts, 'astra_netedit_'+operation)(*arguments[operation])
...
def test_cached_runner_executes_catalog_and_selected_check_without_repository_parents(tmp_path):
    source = Path(__file__).resolve().parents[1]/'plugins/astra-sumo'
...
def test_one_shot_netedit_review_closes_in_the_same_process(monkeypatch, tmp_path, capsys, observed_status):
    ...
    def opened(*args, **kwargs):
        calls.append('open')
        return contracts.AstraToolResult(status='pass',summary='Opened.',payload={'session_id':'owned'})
    def observed(*args, **kwargs):
        calls.append('observe')
        return contracts.AstraToolResult(status=observed_status,summary='Observation result.')
    def closed(*args, **kwargs):
        calls.append('close')
        assert args[0]=='owned' and kwargs['mode']=='abort'
        return contracts.AstraToolResult(status='pass',summary='Closed.')
    monkeypatch.setattr(cli,'astra_netedit_open',opened)
    monkeypatch.setattr(cli,'astra_netedit_observe',observed)
    monkeypatch.setattr(cli,'astra_netedit_close',closed)
...
def test_separate_cli_open_is_rejected_before_launch(monkeypatch, tmp_path, capsys):
    calls = []
    monkeypatch.setattr(cli,'astra_netedit_open',lambda *a,**k: calls.append(a) or contracts.AstraToolResult(status='pass',summary='Opened.'))
