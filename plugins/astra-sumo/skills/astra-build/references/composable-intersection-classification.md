...
| `interaction_kind` | `cross_and_turn`, `merge_diverge`, `access_on_continuous_mainline`, `crossing_only`, `mixed`, `unknown` | Whether paths cross and turn, only merge/split, retain a main-road continuation, or cross without road changes. These are ASTRA projections from ASAM OpenDRIVE interaction types. |
...
## What the sources establish, and what ASTRA infers

The cited sources do not publish one universal ontology. They establish useful
pieces that ASTRA combines. Keep the boundary explicit:

| Source-backed observation | Direct support | ASTRA engineering inference |
| --- | --- | --- |
...
road context. ASTRA may use them as cited candidate features, but must not claim
they are a universal definition of `staggered` or `skew` across all countries.

## ASTRA implementation and compatibility

The generic implementation is
`astra_sumo.intersection.archetype_profile`. It exposes
...
The existing `astra_sumo.intersection.composable_archetype` registry and
...
These sources motivate the representation; the exact ASTRA vocabulary,
thresholds, evidence-status model, and mutation gates are ASTRA engineering
