...
The stable CLI is `plugins/astra-sumo/scripts/build_cached_detector_demand.py`.
It requires:

- the accepted official-TLS manifest (not a loose network file);
- the raw Hamburg count-stream metadata snapshot;
- ASTRA's canonical 15-minute count CSV;
...
```powershell
.venv\Scripts\python.exe plugins\astra-sumo\scripts\build_cached_detector_demand.py `
