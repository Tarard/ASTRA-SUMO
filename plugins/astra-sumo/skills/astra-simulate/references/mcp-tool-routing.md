...
Use this reference when the installed `astra-sumo` plugin is available, when the user asks Codex to run SUMO checks through tools, or when SUMO outputs need interpretation before a code or modeling change.
...
Read `astra workflows --json`, then inspect the selected entry with
`astra workflows --scenario ID --json`. The host model chooses from the
...
and `arguments`. Use `astra workflow selected selection.json --json` to check
...
The legacy `astra_auto_workflow` tool and `run_auto_workflow` also accept
...
|---|---|
| `astra.preflight` | `sumo_preflight` |
| `astra.config.inspect` | `sumo_config_pair_preflight` |
| `astra.run.compare` | `sumo_compare_outputs` |
| `astra.place.resolve` | `sumo_osm_resolve_place` |
| `astra.intersection.classify` | `sumo_intersection_archetype_classify` |
| `astra.signal.classify` | `sumo_signal_device_profile_classify` |
| `astra.network.audit` | `sumo_network_topology_audit`, `sumo_network_connection_mode_audit`, and `sumo_network_overlapping_junction_audit` |
| `astra.network.compare` | `sumo_network_connection_mode_regression_audit` |
| `astra.demand.audit` | `sumo_detector_count_audit` |
| `astra.review.create` | `sumo_network_review_html` |
| `astra.netedit.open/observe/act/close` | `sumo_netedit_session` operations |

`astra.network.audit` has `quick` and `standard` profiles. `quick` runs only
...
| Need full OSM cleanup | CLI: `astra workflow sumo_osm_cleanup_workflow <request.json> --json` | Resolve a place to a bbox first. Use the seven-field request. Reference-matched cleanup is audit-only. Do not present this command as an MCP tool. |
...
| Need NetEdit screenshots or direct mouse/keyboard control | `astra.netedit.open/observe/act/close` | `observe` writes a screenshot and report. Observe again before each action. Use `close(mode=abort)` to discard changes. Use `close(mode=finalize)` only with the latest screenshot SHA-256. |
...
- Low arrived count can mean the horizon is too short, insertion failed, routes are disconnected, or a controller blocked movement. Run `astra network routeability <network.net.xml> <output-dir> --json` before treating a random-route smoke run as usable evidence.
...
- `connected-core` means ASTRA preserved the raw network but routed downstream checks through a netconvert-built largest passenger component with strict connectivity passing.
