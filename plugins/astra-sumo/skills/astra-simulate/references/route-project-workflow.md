...
Use this reference when the user points ASTRA at an existing SUMO/TraCI project, asks what to do next, mixes several requests, or provides partial code, logs, outputs, or results.
...
- road, lane, junction, signal-device, OSM, or construction problem -> `$astra-build`;
- detector, count, route-demand, calibration, or fixed-network replay problem -> `$astra-calibrate`;
- environment, runtime, controller, code, debugging, or experiment-design problem -> stay in `$astra-simulate`;
- completed metrics, comparison, report, claim, or release problem -> `$astra-report`.

For an executable ASTRA operation, read `astra workflows --json`, inspect the chosen scenario, and preserve the user's original target, source authority, and requested action. Missing inputs remain missing. Do not fall back to a different task because one scenario is blocked.
