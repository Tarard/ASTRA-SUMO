"""Small argparse-based ASTRA command line interface.
...
def _exit_code(result: AstraToolResult | dict[str, Any]) -> int:
...
def _emit(result: AstraToolResult | dict[str, Any], *, json_output: bool) -> int:
    if json_output:
        if isinstance(result, AstraToolResult):
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    preflight = subparsers.add_parser("preflight", help="Check Python, SUMO, and ASTRA environment.")
    ...
    mcp = subparsers.add_parser("mcp", help="Run the ASTRA MCP server.")
...
def _dispatch(args: argparse.Namespace) -> int:
    ...
    if args.command == "preflight":
        return _emit(astra_preflight(), json_output=json_output)
    if args.command == "config" and args.config_command == "inspect":
        return _emit(
            astra_config_inspect(args.baseline_config, args.variant_config),
    ...
    if args.command == "place" and args.place_command == "resolve":
        return _emit(astra_place_resolve(args.place_name, limit=args.limit), json_output=json_output)
    ...
    if args.command == "signal" and args.signal_command == "classify":
        return _emit(
            astra_signal_classify(args.ocit_file, expected_node_id=args.expected_node_id),
            json_output=json_output,
        )
    ...
    if args.command == "netedit":
        if args.netedit_command != "review":
            return _emit({"status": "blocked", "error": "Multi-step NetEdit commands require a persistent MCP session. Use 'astra netedit review' for a one-shot CLI review."}, json_output=json_output)
        destination = Path(args.output_dir).resolve()
        if destination.exists():
            raise ValueError("Choose a new NetEdit review output directory.")
        opened = astra_netedit_open(args.source_net_file, str(destination / "candidate.net.xml"), str(destination),
        ...
        try:
            observed = astra_netedit_observe(session_id)
        finally:
            closed = astra_netedit_close(session_id, mode="abort", reason="one_shot_review_complete")
