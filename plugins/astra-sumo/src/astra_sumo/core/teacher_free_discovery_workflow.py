...
def _review_geojson(discovery: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "name": "ASTRA teacher-free OSM signal-cell review",
        "features": features,
    }
...
<title>ASTRA teacher-free discovery</title>
...
def _reset_owned_directory(destination: Path) -> None:
    ...
    if destination.exists() and any(destination.iterdir()):
        if not owner.is_file():
            raise ValueError("Refusing to clear a non-empty discovery directory without ASTRA ownership metadata.")
