    ...
    *,
    ...
    if workflow == "routeability":
        return _blocked(
            ...
            next_question="Which origin and destination should ASTRA snap to passenger-accessible SUMO edges?",
        )
    if workflow == "debug_bad_run":
        return _blocked(
            ...
            next_question="Which SUMO summary, tripinfo, log, or config should ASTRA inspect first?",
        )
    if workflow == "experiment_audit":
        return _blocked(
            ...
            next_question="Which baseline and variant configs or outputs should ASTRA audit as a paired comparison?",
        )
    return _blocked(
        report,
        execution_status="needs_sumo_artifact",
        missing=["sumo_config_or_network_or_outputs"],
        next_question="Which SUMO config, network, route, output, or log should ASTRA inspect?",
    )
    ...
    if osm_file is None:
        ...
        if not source_bbox:
            return _blocked(
                ...
                next_question="Which local OSM intersection patch or bbox should ASTRA compile?",
            )
    ...
    *,
    ...
    if not bbox and not inferred:
        return _blocked(
            ...
            next_question="Which OSM place name or bbox should ASTRA use?",
        )
    if not bbox:
        ...
        if candidate.get("status") != "pass" or not resolved_bbox:
            ...
            report["next_question"] = "Which OSM area or bbox should ASTRA use?"
            return report
    ...
    *,
    ...
    if net_file is None:
        return _blocked(
            ...
            missing=["net_file"],
            next_question="Which SUMO .net.xml should ASTRA use for the TLS review?",
        )
    ...
    *,
    ...
    if net_file is None:
        return _blocked(
            ...
            execution_status="needs_network",
            missing=["net_file"],
            next_question="Which partial SUMO .net.xml should ASTRA use for the HTML review cockpit?",
        )
