...
from astra_sumo.core.artifact_io import write_json_atomic
from astra_sumo.core.digital_twin_workflow import prepare_hamburg_corridor_digital_twin
...
from astra_sumo.core.hamburg_named_replay import materialize_hamburg_named_replay
from astra_sumo.core.hamburg_named_signal_observations import (
    materialize_hamburg_named_signal_observations,
)
from astra_sumo.core.digital_twin import parse_iso_datetime
from astra_sumo.core.hamburg_execution_workflow import (
    materialize_hamburg_execution_plan,
)
from astra_sumo.core.hamburg_official import SensorThingsClient
from astra_sumo.core.hamburg_corridor_workflow import (
    prepare_hamburg_sandtorkai_geometry_safe_corridor_package,
)
from astra_sumo.core.hamburg_official import SANDTORKAI_THREE_INTERSECTIONS
from astra_sumo.core.official_tls_workflow import rebuild_hamburg_sandtorkai_official_tls
from astra_sumo.core.route_sampler import run_route_sampler
...
from astra_sumo.core.tls_replay import run_tls_detector_replay
from astra_sumo.intersection.composable_archetype import build_hamburg_2394_archetype_profile
...
def sumo_hamburg_sandtorkai_digital_twin(
    ...
    When ``net_file`` is omitted, ASTRA first downloads/reuses OSM for the fixed
    corridor bbox and runs its cleanup gates.  Supplying an already frozen ASTRA
    ...
    try:
        ...
        if net_file:
            ...
            network_path = _existing_file(str(network_gate["net_file"]), "ASTRA cleanup net_file")
        ...
        if not net_file:
            ...
            if network_gate["status"] == "provisional":
                ...
                gaps.append(
                    "ASTRA structural build is usable, but its map-review gates remain provisional; "
        ...
        report["workflow"] = (
            "ASTRA OSM cleanup -> compact three-intersection scope -> Hamburg official "
...
def _torii_network_gate(report: dict[str, Any]) -> dict[str, Any]:
    """Allow official MAP/OCIT validation to follow a sound but review-pending ASTRA build."""
    ...
    if failed_required or not net_file or not Path(net_file).is_file():
        return {
            ...
            "reason": "ASTRA base network is not structurally ready for Hamburg official-data validation",
        }
    ...
    return {
        ...
        "policy": (
            ...
            "until every delegated ASTRA review gate is resolved"
        ),
    }
