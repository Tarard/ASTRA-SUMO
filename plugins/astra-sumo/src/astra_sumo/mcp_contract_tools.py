...
keep external URLs, executable paths, and most output locations under ASTRA's
control and return one structured result shape.  They call the same business
functions as the legacy MCP tools and the ``astra`` CLI.
"""
...
class AstraToolResult(BaseModel):
    ...
    artifacts: list[AstraArtifact] = Field(default_factory=list)
...
def astra_preflight() -> AstraToolResult:
    raw = sumo_preflight()
...
def astra_config_inspect(
    baseline_config: Annotated[
        str,
        Field(description="Path to the baseline .sumocfg file; ASTRA does not modify it."),
    ],
    variant_config: Annotated[
        str,
        Field(description="Path to the variant .sumocfg file; ASTRA does not modify it."),
    ],
) -> AstraToolResult:
    ...
    return _result(
        ...
        next_actions=(
            ...
            else ["Run astra run compare against the inspected configuration pair."]
        ),
    )


def astra_run_compare(
...
def astra_intersection_classify(
