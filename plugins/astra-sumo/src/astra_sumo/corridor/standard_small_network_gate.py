...
from astra_sumo.core.standard_nema_binding import (
    ...
    report = {
        ...
        "interpretation": {
            ...
            "negative": (
                ...
                "fixtures but remain outside ASTRA's automatic candidate "
                "envelope because they contain U-turns and shared "
                "left/through lanes."
            ),
            "no_contradiction": (
                "An official SUMO example can be valid while ASTRA correctly abstains from rewriting it."
            ),
        },
...
def _reset_owned_output_directory(destination: Path) -> None:
    ...
    if destination.exists() and any(destination.iterdir()):
        if not owner.is_file():
            raise ValueError(
                "Refusing to clear a non-empty small-network gate directory without ASTRA ownership metadata."
            )
