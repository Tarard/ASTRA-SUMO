...
def validate_intersection(
    ...
    for warning in ir.movement_matrix.restriction_warnings:
        _add_warning(warnings, warning_records, warning, "astra", "diagnostic")
    ...
    if artifacts.net_file and net_path.exists():
        ...
        _add_warning(warnings, warning_records, "compiled net file not available", "astra", "blocking")
    if artifacts.net_file and net_path.is_file():
        for warning in _compiled_net_edge_warnings(ir, net_path):
            _add_warning(warnings, warning_records, warning, "astra", "blocking")
    ...
    if needs_sumo_crossing(ir) and not _net_has_crossing(net_path):
        _add_warning(warnings, warning_records, "missing SUMO crossing edge for OSM pedestrian crossing support", "astra", "blocking")
