...
from .tools.workflow_tools import astra_auto_workflow
...
def register_legacy_mcp_tools(server: FastMCP, *, torii_names: bool = False) -> None:
    ...
    server.tool(name="torii_auto_workflow" if torii_names else "astra_auto_workflow", description="Execute an explicit host-LLM workflow_selection from the registered scenario catalog without keyword reclassification. Inspect-only and ask-first retain a plan. Without workflow_selection, retain the older natural-language router for compatibility. Discover scenarios with astra workflows --json.")(
        astra_auto_workflow
    )
    ...
    server.tool(description="Build the reusable Am Sandtorkai three-intersection vehicle digital-twin package: ASTRA OSM cleanup, official Hamburg count and traffic-light API snapshots, same-location SUMO E1/E2 sensors, 15-minute edge constraints, routeSampler demand, and auditable TLS link events.")(
        sumo_hamburg_sandtorkai_digital_twin
    )
