...
from astra_sumo.core.artifact_io import write_json_atomic, write_text_atomic
...
from astra_sumo.core.sumo_commands import run_sumo_load_audit
from astra_sumo.core.surface_overlap_audit import audit_sumo_lane_junction_surface_overlaps
...
def _build_axis_fragments(
    ...
    *,
    ...
    for source in root.findall("edge"):
        ...
        direction = str(params.get("astra:station_direction", ""))
        ...
        if corridor_id not in by_corridor:
            ...
            continue
        station_from = float(params["astra:station_from_m"])
        station_to = float(params["astra:station_to_m"])
        ...
        for index, (left, right) in enumerate(zip(ordered, ordered[1:])):
            ...
            _set_param(item, "astra:station_from_m", str(_rounded(left)))
            _set_param(item, "astra:station_to_m", str(_rounded(right)))
            _set_param(item, "astra:splice_source_edge", edge_id)
...
def _add_core_clearance_intervals(
    ...
    for source in root.findall("edge"):
        ...
        corridor_id = _corridor_id(
            str(params.get("origId", "")), str(params.get("astra:station_direction", ""))
        )
...
def _axis_node_at_station(
    ...
    *,
    ...
    direction = _params(source).get("astra:station_direction", "")
    station_from = float(_params(source)["astra:station_from_m"])
    station_to = float(_params(source)["astra:station_to_m"])
...
def _station_at_source_point(source: ET.Element, point: Sequence[float]) -> float:
    ...
    params = _params(source)
    start = float(params["astra:station_from_m"])
    end = float(params["astra:station_to_m"])
    raw = projection["fraction"] if params.get("astra:station_direction") == "with_stationing" else 1.0 - projection["fraction"]
...
def _station_fraction(source: ET.Element, station: float) -> float:
    ...
    return max(0.0, min(1.0, raw if params.get("astra:station_direction") == "with_stationing" else 1.0 - raw))
